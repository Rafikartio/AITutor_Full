...
tutor_logic.py
--------------
Core logic for the AI Study Coach app: data validation, rule-based
priority scoring, KMeans clustering, recommendation generation, and
evaluation.

Kept separate from app.py (the UI) so logic and interface stay
independent of each other — a "modular code" requirement.
"""
...
def validate_data(df):
    """Check the dataframe for missing/invalid values. Returns a list of error strings."""
    errors = []
    ...
    if df.empty:
        errors.append("No data provided. Add at least one subject to continue.")
        return errors

    if df["subject"].isnull().any():
        errors.append("Every row needs a subject name.")
    ...
    if df["hours_studied"].isnull().any() or (df["hours_studied"] < 0).any():
        errors.append("hours_studied can't be negative or empty.")
...
def preprocess_data(df):
    """Clean the dataframe and enforce correct types."""
    df = df.copy()
...
def calculate_priority_score(row, weight_score=0.5, weight_attempts=8, weight_hours=3):
    ...
    A higher priority score means the subject needs more attention.
    """
...
def run_rule_based(df):
    """Apply rule-based priority scoring across the whole dataframe."""
    df = df.copy()
...
def run_kmeans_clustering(df, n_clusters=3, random_state=42):
    ...
    student — not enough labeled examples to properly train/test a
    supervised classifier. KMeans finds natural groupings without needing
    labeled training data, which suits small, single-student datasets.
    """
    df = df.copy()
    ...
    # Label clusters by average quiz_score: the lowest-scoring cluster becomes "Weak"
...
def generate_explanation(top_row):
    ...
    explanation = (
        ...
        f"Why: your score here is {top_row['quiz_score']:.0f}% "
        f"(classified as '{top_row['performance_level']}' performance). "
        f"You attempted it {int(top_row['attempts'])} time(s) and studied it for "
        f"{top_row['hours_studied']:.1f} hour(s). "
        f"That combination gives it the highest priority score of {top_row['priority_score']}, "
        f"which means the effort spent here isn't matching results yet compared to your other subjects."
    )
    return explanation
...
def generate_practice_questions(subject):
    """Return simple starter practice tasks for the recommended subject."""
    templates = [
        f"Review the core definitions and key terms in {subject}.",
        f"Attempt 5 practice problems focused on {subject} fundamentals.",
        f"Re-read or re-watch the lesson material for {subject}, then summarise it in your own words.",
    ]
    return templates
